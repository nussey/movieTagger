...
"  --hdvideo      (number|bool)Set the hdvideo flag (true, false, 0=sd, 1=720p, 2=1080p etc...)\n"
...
"  --flavour       (string)    Sets the \"flvr\" atom\n"
"\n"
"Deleting tags\n"
    ...
    for (int i= 0; i <= max_optargs-1; i++) {
        if ( argv[start_optindargs + i] && start_optindargs + i <= total_args ) {
            if ( strncmp(argv[start_optindargs + i], "lang=", 5) == 0 ) {
                ...
                trk_idx = uint8_from_string(track_index_str);
...
//        { "performer",        required_argument,  NULL,           Meta_performer },
        ...
        { "flavour",          required_argument,  NULL,           Meta_Flavour } ,
...
*/
        ...
        case Meta_tracknum : {
            ...
            if (strrchr(optarg, '/') != NULL) {
                ...
                pos_in_total = uint16_from_string(item_stat);
                item_stat = strsep(&duplicate_info,"/");
                the_total = uint16_from_string(item_stat);
            } else {
                pos_in_total = uint16_from_string(optarg);
            }
        ...
        case Meta_hdvideo : {
            ...
            if (strcmp(optarg, "false") == 0) {
                ...
                uint8_t hdvid = uint8_from_string(optarg);
                //compilation: [0, 0, 0, 0,   boolean_value]; BUT that first uint32_t is already accounted for in APar_MetaData_atom_Init
                AtomicInfo* hdvideoData_atom = APar_MetaData_atom_Init("moov.udta.meta.ilst.hdvd.data", optarg, AtomFlags_Data_UInt);
                APar_Unified_atom_Put(hdvideoData_atom, NULL, UTF8_iTunesStyle_256glyphLimited, hdvid, 8);
            }
        ...
        case Meta_BPM : {
            ...
            if (strcmp(optarg, "0") == 0 || strlen(optarg) == 0) {
                ...
                bpm_value = uint16_from_string(optarg);
        ...
        case Meta_TV_SeasonNumber : { //if the show "ABC Lost 209", its 2; integer 2 not char "2"
			...
			uint16_t data_value=uint16_from_string(optarg);
        ...
        case Meta_cnID : { // the iTunes Catalog ID
			...
			uint32_t data_value=uint32_from_string(optarg);
		...
		case Meta_plID : { // the iTunes Playlist ID
            ...
            uint64_t data_value=uint64_from_string(optarg);
        ...
        case Meta_SortOrder : {
            ...
            else if (strncmp(optarg,"artist=",7)==0) {
                type="artist";
                value=optarg+7;
            }
            else if (strncmp(optarg,"albumartist=",12)==0) {
                type="albumartist";
                value=optarg+12;
            }
            else if (strncmp(optarg,"album=",6)==0) {
                type="album";
                value=optarg+6;
            }
            else if (strncmp(optarg,"composer=",9)==0) {
                type="composer";
                value=optarg+9;
            }
            else if (strncmp(optarg,"show=",5)==0) {
                type="show";
                value=optarg+5;
            }
            else {
            ...
            if ( strcmp(type, "name") == 0 ) {
                sortOrder_atom = APar_MetaData_atom_Init("moov.udta.meta.ilst.sonm.data", value, AtomFlags_Data_Text);
            } else if ( strcmp(type, "artist") == 0 ) {
                sortOrder_atom = APar_MetaData_atom_Init("moov.udta.meta.ilst.soar.data", value, AtomFlags_Data_Text);
            } else if ( strcmp(type, "albumartist") == 0 ) {
                sortOrder_atom = APar_MetaData_atom_Init("moov.udta.meta.ilst.soaa.data", value, AtomFlags_Data_Text);
            } else if ( strcmp(type, "album") == 0 ) {
                sortOrder_atom = APar_MetaData_atom_Init("moov.udta.meta.ilst.soal.data", value, AtomFlags_Data_Text);
            } else if ( strcmp(type, "composer") == 0 ) {
                sortOrder_atom = APar_MetaData_atom_Init("moov.udta.meta.ilst.soco.data", value, AtomFlags_Data_Text);
            } else if ( strcmp(type, "show") == 0 ) {
                sortOrder_atom = APar_MetaData_atom_Init("moov.udta.meta.ilst.sosn.data", value, AtomFlags_Data_Text);
            }
            APar_Unified_atom_Put(sortOrder_atom, value, UTF8_iTunesStyle_256glyphLimited, 0, 0);
        ...
        case Meta_Flavour : {
            APar_ScanAtoms(ISObasemediafile);
            if ( !APar_assert(metadata_style == ITUNES_STYLE, 1, "Flavour") ) {
                break;
            }

            AtomicInfo* flavourData_atom = APar_MetaData_atom_Init("moov.udta.meta.ilst.flvr.data", optarg, AtomFlags_Data_Text);
            APar_Unified_atom_Put(flavourData_atom, optarg, UTF8_iTunesStyle_256glyphLimited, 0, 0);
            break;
        }
